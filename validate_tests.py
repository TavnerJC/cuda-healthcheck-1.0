...
def main():
    ...
    results.append(run_command(["pytest", "tests/test_logging.py", "-v"], "Logging tests"))
    results.append(run_command(["pytest", "tests/test_retry.py", "-v"], "Retry tests"))
    results.append(run_command(["pytest", "tests/test_exceptions.py", "-v"], "Exception tests"))

    # Test 2: Run orchestrator tests
    print_header("Test Suite 2: HealthcheckOrchestrator")
    results.append(
        run_command(["pytest", "tests/test_orchestrator.py", "-v"], "Orchestrator tests")
    )

    # Test 3: Run breaking changes tests
    print_header("Test Suite 3: Breaking Changes Database")
    results.append(
        run_command(["pytest", "tests/test_breaking_changes.py", "-v"], "Breaking changes tests")
    )

    # Test 4: Run databricks integration tests (with mocks)
    print_header("Test Suite 4: Databricks Integration (Mocked)")
    results.append(
        run_command(["pytest", "tests/databricks/", "-v"], "Databricks integration tests")
    )
